...
const setDeliveryStaffOrder = async (req, res) => {
    ...
    try {
        ...
        if (!deliveryStaff || !order) {
            return res.json({ success: false, message: "Delivery staff/Order not found" });
        }
        ...
        const token = jwt.sign({ deliveryStaffId: deliveryStaff._id, orderId: order._id }, process.env.JWT_SECRET, {
            expiresIn: "7d",
        });
...
const confirmOrderDeliveredSuccessfully = async (req, res) => {
    ...
    try {
        ...
        const deliveryStaff = await deliveryStaffModel.findById(verify.deliveryStaffId);
        ...
        if (order.status === "Delivered" || (order.status === "Cancelled")) {
            res.render("resultAfterConfirmOrderDelivery", { success: false, message: "Open link to see the result" })
        } else {
            res.render("confirmOrderDelivered", { success: false, message: "Open link to confirm order delivered" });
        }
    } catch (error) {
        console.log(error);
        res.json({ success: false, message: "Error" });
...
const confirmOrderDelivered = async (req, res) => {
    ...
    try {
        ...
        res.render("confirmOrderDelivered", { success: true, message: "Confirm order delivered" });
    } catch (error) {
        console.log(error);
        res.json({ success: false, message: "Error" });
    }
...
const getNameDeliveryStaffFromOrderId = async (req, res) => {
    ...
    try {
        const deliveryStaffOrder = await deliveryStaffOrderModel.findOne({ orderId: orderId })

        if (!deliveryStaffOrder) {
            return res.json({ success: false, message: "Delivery Staff Order not FOUND" })
        }

        const deliveryStaff = await deliveryStaffModel.findById(deliveryStaffOrder.deliveryStaffId)
...
export { setDeliveryStaffOrder, confirmOrderDeliveredSuccessfully, confirmOrderDelivered, getNameDeliveryStaffFromOrderId };