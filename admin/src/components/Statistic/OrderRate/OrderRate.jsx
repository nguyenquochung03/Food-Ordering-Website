    ...
    try {
      ...
      if (selectedMonth) {
        ...
        const response = await axios.get(
          `${url}/api/order/getTotalOrdersByMonth?year=${year}`
        );
        if (response.data.success) {
          const [currentYearData, previousYearData] = response.data.data;
          const months = currentYearData.map((item) => item.month);
          const currentYearOrders = currentYearData.map(
            (item) => item.totalOrders
          );
          const previousYearOrders = previousYearData.map(
            (item) => item.totalOrders
          );
  ...
  const fetchTotalOrdersByYear = async () => {
    try {
      ...
      if (selectedMonth) {
        const [year] = selectedMonth.split("-");
        const response = await axios.get(
          `${url}/api/order/getTotalOrdersByYear?year=${year}`
        );
        if (response.data.success) {
          setOrderStatistic((prev) => ({
            ...prev,
      ...
      <div className="order-rate-header">
        <div className="order-rate-header-left">
          ...
          <div className="order-rate-header-left-order-total">
            <p>Order Total</p>
            <p>
              <span>{orderStistic.totalOrders}</span>
            </p>
          </div>
        </div>
