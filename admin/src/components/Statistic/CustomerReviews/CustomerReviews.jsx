...
const CustomerReviews = ({ setIsLoading, url, selectedMonth }) => {
  ...
  const [weeklyHighRatingComment, setWeeklyHighRatingComment] = useState(
    Array(7).fill(0)
  );
  const [weeklyLowRatingComment, setWeeklyLowRatingComment] = useState(
    Array(7).fill(0)
  );
  ...
  const fetchPositiveTotal = async () => {
    try {
      ...
      const response = await axios.get(
        `${url}/api/comment/getHighRatingCommentTotals`,
        {
          params: { month, year },
        }
      );
      ...
      if (response.data.success) {
        const currentMonthHighRatingCount =
          response.data.data.currentMonthHighRatingCount;
        const previousMonthHighRatingCount =
          response.data.data.previousMonthHighRatingCount;
  ...
  const fetchNegativeTotal = async () => {
    try {
      ...
      const [year, month] = selectedMonth.split("-");
      const response = await axios.get(
        `${url}/api/comment/getLowRatingCommentTotals`,
        {
          params: { month, year },
        }
      );
      ...
      if (response.data.success) {
        const currentMonthLowRatingCount =
          response.data.data.currentMonthLowRatingCount;
        const previousMonthLowRatingCount =
          response.data.data.previousMonthLowRatingCount;
  ...
  const fetchWeeklyHighRatingComment = async () => {
    try {
      ...
      if (selectedMonth) {
        const [year, month] = selectedMonth.split("-");
        const response = await axios.get(
          `${url}/api/comment/getWeeklyHighRatingCommentTotals`,
          {
            params: { month, year },
          }
        );
  ...
  const fetchWeeklyLowRatingComment = async () => {
    try {
      ...
      if (selectedMonth) {
        const [year, month] = selectedMonth.split("-");
        const response = await axios.get(
          `${url}/api/comment/getWeeklyLowRatingCommentTotals`,
          {
            params: { month, year },
          }
        );
      ...
      <div className="customer-reviews-container">
        <div className="customer-reviews">
          <div className="customer-reviews-total">
            ...
            <p className="customer-reviews-total-this-month-value">
              {positiveTotal} <span>Reviews</span>
            </p>
            <div className="customer-reviews-total-percentage">
              <img
                src={
                  positivePercentage > 0 ? images.arrow_up : images.arrow_down
                }
                alt=""
              />
              <p
            ...
            {weeklyHighRatingComment.map((count, index) => (
              ...
              >
                ...
                <div
                  ...
                  style={{
                    height: `${
                      1 + count.total * 3 > 80 ? 80 : 1 + count.total * 3
                    }px`,
                    backgroundColor:
                      hoverIndex === index
                        ? index % 2 === 0
        ...
        <div className="customer-reviews">
          <div className="customer-reviews-total">
            ...
            <p className="customer-reviews-total-this-month">This Month</p>
            <p className="customer-reviews-total-this-month-value">
              {negativeTotal} <span>Reviews</span>
            </p>
            <div className="customer-reviews-total-percentage">
              <img
                src={
                  negativePercentage > 0 ? images.arrow_up : images.arrow_down
                }
                alt=""
              />
              <p
