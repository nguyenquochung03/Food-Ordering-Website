...
const RecentReviews = ({ setIsLoading, selectedMonth, url }) => {
    ...
    useEffect(() => {
        ...
        const negReviews = recentReviews.filter(data => data.rating <= 3);
    ...
    async function fetchRecentReview() {
        try {
            ...
            if (selectedMonth) {
                ...
                const response = await axios.get(`${url}/api/comment/getCommentsSorted/`, {
                    params: { month, year }
                });
                                ...
                                <div className="recent-reviews-item" key={data._id}>
                                    <div className="recent-reviews-item-top">
                                        ...
                                        <div className="recent-reviews-item-top-right">
                                            <p>{data.user.name}</p>
                                            <p><span>User since {moment(data.user.date).year()}</span></p>
                                        </div>
                                    </div>
                                    <hr />
                                    <div className="recent-reviews-item-bottom">
                                        <div className="recent-reviews-item-bottom-top">
                                            ...
                                            <div className='recent-reviews-item-bottom-top-right'>
                                                ...
                                                <div className="recent-reviews-food-item-rating">
                                                    {[...Array(5)].map((_, i) => {
                                                        const isHalfStar = data.rating > i && data.rating < i + 1;

                                                        return (
                                                            <span key={i} className="recent-reviews-food-item-rating-star">
                                                                {isHalfStar ? (
                                                                    <span className="recent-reviews-food-item-rating-half-star">
                                                                        <span className="recent-reviews-full-star">★</span>
                                                                        <span className="recent-reviews-empty-star">☆</span>
                                                                    </span>
                                                                ) : (
                                                                    i < data.rating ? '★' : '☆'
                                                                )}
                                        ...
                                        <p className='recent-reviews-item-bottom-describe'>{data.describe}</p>
                                    </div>
                                </div>
                            ))
                        }
