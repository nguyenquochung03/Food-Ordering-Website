...
const StaffDelivery = ({ url, setIsLoading }) => {
    ...
    const removeDeliveryStaff = async (id) => {
        if (window.confirm("Are you sure you want to delete this staff?")) {
            try {
                setIsLoading(true)
                const response = await axios.post(`${url}/api/deliveryStaff/delete/`, { id: id });
                await fetchList();
    ...
    return (
        <React.Fragment>
            {
                loading ? <SkeletonLoadingListAdd /> :
                    (
                        isAdd ? <AddDeliveryStaff url={url} setIsAdd={setIsAdd} setIsLoading={setIsLoading} fetchList={fetchList} /> :
                            isUpdate ? <UpdateDeliveryStaff url={url} dataUpdate={dataUpdate} setIsUpdate={setIsUpdate} setIsLoading={setIsLoading} fetchList={fetchList} /> :
                                <div className='delivery-staff'>
                                    <div className="delivery-staff_add">
                                        <button onClick={() => setIsAdd(true)} type='button'><img src={images.add_icon} alt="" />Add</button>
                                    </div>
                                            ...
                                            {filterListDeliveryStaff.map((item, index) => (
                                                <div key={index} className='list-table-format'>
                                                    ...
                                                    <div className="list-table-format-action">
                                                        <button onClick={() => updateDeliveryStaff(item)} className='edit' type='button'>Edit</button>
                                                        <button onClick={() => removeDeliveryStaff(item._id)} className='remove'>Remove</button>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                    <NormalPagination food_list={listDeliveryStaff} setList={setFilterListDeliveryStaff} setIsLoading={setIsLoading} />
                                </div>
                    )
            }
        </React.Fragment>
