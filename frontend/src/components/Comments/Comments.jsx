...
const Comment = ({
  ...
  const handleLike = async () => {
    if (token) {
      ...
      const result = window.confirm(
        "Log in to comment or interact with other users"
      );
  ...
  const onPostCommentHandler = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        { foodId: foodId, parentId: idComment, describe: replyContent },
        { headers: { token } }
      );
      if (response.data.success) {
        fetchComments();
  ...
  const onEditCommentHandler = async () => {
    ...
    try {
      setIsLoading(true);
      const response = await axios.post(`${url}/api/comment/edit`, {
        commentId: idComment,
    ...
    <React.Fragment>
        ...
        <div className="comment-content">
          <div className="comment-content-header">
            <div className="comment-content-header-name-rating">
              ...
              {isAdmin && (
                <p className="comment-content-header-name-admin">Admin</p>
              )}
            </div>
            <div className="comment-content-time-edit-remove">
              ...
              {isUser && (
                <div className="comment-content-header-you">
                  <p>You</p>
                  <div
                    className="comment-content-header-you-edit"
                    onClick={() => setIsEditing((prevState) => !prevState)}
                  >
                    <EditIcon color={"#e3a239"} size={16} />
                  </div>
                  <div
                    className="comment-content-header-you-remove"
            ...
            {parentId !== "none" && parentName && (
              <p className="comment-content-header-parent-name">
          ...
          {isEditing && (
            <div className="comment-edit">
              ...
              <div className="reply-comment-btn">
                <button
                  onClick={onCancelEditCommentHandler}
          ...
          <div className="comment-footer">
            <div
              className="comment-footer-num-reply"
              onClick={() => toggleShowReply("none")}
            >
              <img
                src={
                  isShowReply
                    ? "https://rvs-comment-module.vercel.app/Assets/Down.svg"
                    : "https://rvs-comment-module.vercel.app/Assets/Up.svg"
                }
                alt=""
              />
              <p>
      ...
      {isRepling && (
        <div className="reply-comment">
          ...
          />
          <div className="reply-comment-btn">
            <button onClick={onCancelCommentHandler} className="btn-cancel">
              Cancel
            </button>
            <button
...
const Comments = ({
  ...
  const rootComments = Object.values(commentMap).filter(
  ...
  const renderComments = (comment, parentName) => (
    <li key={comment.idComment}>
      ...
      {comment.children &&
        ...
        isShowReply[comment.idComment] && (
          <ul>
            {comment.children.map((child) =>
              renderComments(child, comment.nameUser)
            )}
          </ul>
        )}
    </li>
  ...
  return (
    <section className="comment-module">
      <ul>{rootComments.map((comment) => renderComments(comment, ""))}</ul>
    </section>
  );
};
